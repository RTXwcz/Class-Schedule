# 文档导航

## 开始使用

- [项目说明与下载](../README.md)
- [Android 构建与签名](../apk/构建APK.md)
- [Web/PWA 安装](../打包说明.md)
- [版本记录](../CHANGELOG.md)
- [贡献指南](../CONTRIBUTING.md)

## 开发与设计

- [原生应用设计](superpowers/specs/2026-09-06-class-schedule-native-design.md)
- [实现计划及当前完成状态](superpowers/plans/2026-09-06-class-schedule-native-plan.md)
- [移动端中文课表 OCR 调研](research/2026-09-07-mobile-chinese-timetable-ocr.md)
- [第三方许可与来源](../THIRD_PARTY_NOTICES.md)
- [原始上课时间参考](reference/上课时间.docx) / [图标原稿](reference/图标原稿.jpg)

## 验证

- [核心功能最终验证](superpowers/verification/2026-09-07-native-final-verification.md)
- [界面、可选单双周与镜像下载验证](superpowers/verification/2026-09-07-usability-verification.md)
- [v1.3.0 发布记录](releases/v1.3.0.md)

`superpowers/` 下的设计、计划和早期验证文档保留当时上下文；当前构建步骤以 README 和构建说明为准，发布状态以 Releases 为准。
