...
package com.kebiao.app.ocr.paddle.postprocess
...
object CTCDecoder {
    ...
    fun decode(output: FloatArray, shape: LongArray, characterList: List<String>): List<Pair<String, Float>> {
        require(shape.size == 3 && shape.all { it > 0 }) { "Invalid OCR recognition output shape" }
        val batchSize = shape[0].toInt()
        val timeSteps = shape[1].toInt()
        val numClasses = shape[2].toInt()
        require(numClasses == characterList.size + 1) { "OCR model and dictionary do not match" }
        require(output.size.toLong() == shape.reduce { a, b -> a * b }) { "Incomplete OCR output" }
