...
class ORTSessionManager(
    ...
    fun loadModels(detAssetPath: String, recAssetPath: String) {
        ...
        try {
            ...
            require(File(detAssetPath).isAbsolute && File(detAssetPath).isFile)
            require(File(recAssetPath).isAbsolute && File(recAssetPath).isFile)
            try {
                detSession = ortEnv.createSession(detAssetPath, opts)
            } catch (t: Throwable) {
                throw OCRError.ModelLoadFailed("detection", t)
            }
            try {
                recSession = ortEnv.createSession(recAssetPath, opts)
            } catch (t: Throwable) {
                detSession?.close()
                detSession = null
